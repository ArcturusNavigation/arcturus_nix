...
# Specify a launch file in `all_seaing_bringup` to start (do not include the `launch.py`).
...
# For running other launch configurations in a similarly reproducible environment, use `nix develop -i --keep HOME`
# and then run `. ./source.sh` followed by the ROS launch commands.

file=${1:-"vehicle-nix-test"}
nix develop -i --keep HOME --command bash -c ". ./source.sh; ros2 launch all_seaing_bringup ${file}.launch.py"
